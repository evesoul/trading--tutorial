...
| `StageBadge` | 阶段徽章：导学 / 指标 / 组合 / 交易系统 / 威科夫与量价 |
| `StageMap` | 四阶段各一句 |
| `PathSteps` | 阶段 0 / 推荐主路径 / 合约数据层 / 指标组合 / 交易系统 / 威科夫；未发布只显示「编写中」 |
| `LessonPager` | 上一篇 / 下一篇，未发布用 `resolvePublishedPath` |
| `PrerequisiteList` | 先修芯片 |
| `EmptyState` | 组合 / 系统 / 术语空状态 |
| `ButtonLink` | 主按钮、次要按钮 |
| `SiteFooter` | 页脚风险与教育用途说明 |
| `courseMeta.ts` | 导航、阶段、阶段 0、主路径、主路径续、对照层、合约数据层、组合、系统与威科夫 slug、未发布标题 |

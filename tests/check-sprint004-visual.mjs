/**
 * Sprint 004 视觉抽查：本机 Chrome headless + CDP。
 * 需要预览已在 BASE_URL 运行。
 * 用法：BASE_URL=http://127.0.0.1:3000 node tests/check-sprint004-visual.mjs
 */
...
const outDir = process.env.QA_SHOT_DIR ?? join(tmpdir(), 'qa-sprint004')
...
const chromeProfile = await mkdtemp(join(tmpdir(), 'chrome-qa-s004-'))
...
try {
  ...
  const course = await cdp.evaluate(`(() => {
    ...
    return {
      items,
      hasCombinationsHref: html.includes('href="/combinations/'),
      hasHandoff: html.includes('HANDOFF'),
      stepCount: items.length,
    }
  })()`)

  if (course.stepCount !== 12) {
    fail(`/course 路径步数是 ${course.stepCount}，应为 8 + 4`)
  }
  else {
    ok('/course 路径步数 12（主路径 8 + 合约层 4）')
  }
  if (course.hasCombinationsHref) {
    fail('/course 含 href="/combinations/')
  }
  ...
  for (const [i, href] of expectedHrefs.entries()) {
    ...
    const expectedLabel = i < 8 ? `主路径 ${i + 1}` : `合约数据层 ${i - 7}`
  ...
  const figurePages = [
    {
      path: '/indicators/open-interest',
      vis: 'vis-006',
      altNeed: '示意图',
      shot: 'desktop-oi.png',
    },
    {
      path: '/indicators/funding-rate',
      vis: 'vis-007',
      altNeed: '示意图',
      shot: 'desktop-funding.png',
    },
    {
      path: '/indicators/long-short-ratio',
      vis: 'vis-011',
      altNeed: '示意图',
      extraAlt: '口径',
      shot: 'desktop-lsr.png',
    },
    {
      path: '/indicators/cvd',
      vis: 'vis-012',
      altNeed: '示意图',
      shot: 'desktop-cvd.png',
      noCombinations: true,
    },
  ]
  ...
  for (const item of figurePages) {
    ...
    const page = await cdp.evaluate(`(() => {
      ...
      return {
        ...
        hasCombinations: html.includes('href="/combinations/'),
        writing: document.body.innerText.includes('这篇指标课还没发布'),
      }
    })()`)
    if (!page.src || !page.alt.includes(item.altNeed) || page.writing) {
      ...
      fail(`${item.path} 图 alt 缺口径：${JSON.stringify(page)}`)
    }
    else if (item.noCombinations && page.hasCombinations) {
      fail(`${item.path} 含 href="/combinations/`)
    }
    else {
  ...
  const bb = await cdp.evaluate(`(() => {
    const html = document.documentElement.outerHTML
    return {
      hasOiHref: html.includes('href="/indicators/open-interest"'),
    }
  })()`)
  if (!bb.hasOiHref) {
    fail('布林带页不可点 OI')
...
console.log('\nAll Sprint 004 visual checks passed')

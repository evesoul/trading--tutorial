/**
 * Sprint 006 视觉抽查：本机 Chrome headless + CDP。
 * 需要预览已在 BASE_URL 运行。
 * 用法：BASE_URL=http://127.0.0.1:3000 node tests/check-sprint006-visual.mjs
 */
...
const outDir = process.env.QA_SHOT_DIR ?? join(tmpdir(), 'qa-sprint006')
...
const chromeProfile = await mkdtemp(join(tmpdir(), 'chrome-qa-s006-'))
...
try {
  ...
  const course = await cdp.evaluate(`(() => {
    ...
    return {
      items,
      unpublishedHrefs: ${JSON.stringify(unpublishedHrefs)}.filter(href => html.includes('href="' + href + '"')),
  ...
  if (course.unpublishedHrefs.length) {
    fail(`/course 含未发布死链 ${course.unpublishedHrefs.join(', ')}`)
  }
  ...
  const writingSteps = course.items.slice(27)
  ...
  for (const [i, title] of expectedWriting.entries()) {
    const step = writingSteps[i]
    const expectedLabel = `交易系统 ${i + 9}`
    if (!step || !step.writing || step.href || step.title !== title || step.label !== expectedLabel) {
      fail(`/course 后 7 步第 ${i + 1} 应只标编写中：${JSON.stringify(step)}`)
    }
    else {
      ok(`/course ${step.label} ${step.title}（无死链）`)
    }
  }
  ...
  await goto(cdp, '/combinations/multi-indicator')
  const resonance = await cdp.evaluate(`(() => {
    const html = document.documentElement.outerHTML
    const link = document.querySelector('a[href="/trading-system/what-is-a-trading-system"]')
    return {
      hasHref: html.includes('href="/trading-system/what-is-a-trading-system"'),
      linkText: link?.textContent?.trim() ?? '',
    }
  })()`)
  if (!resonance.hasHref) {
    fail('共振页不可点 /trading-system/what-is-a-trading-system')
  }
  else {
    ok(`共振可点 what-is-a-trading-system（${resonance.linkText || '有 href'}）`)
  }
  await screenshot(cdp, 'desktop-multi-indicator.png')

  const figurePages = [
    {
      path: '/trading-system/what-is-a-trading-system',
      vis: 'vis-008',
      altNeed: '示意图',
      shot: 'desktop-what-is-a-trading-system.png',
    },
    {
      path: '/trading-system/market-regime',
      vis: 'vis-017',
      altNeed: '示意图',
      extraAlt: '市场环境',
      shot: 'desktop-market-regime.png',
    },
    {
      path: '/trading-system/stop-loss',
      vis: 'vis-018',
      altNeed: '示意图',
      extraAlt: '强平',
      shot: 'desktop-stop-loss.png',
    },
    {
      path: '/trading-system/position-sizing',
      vis: 'vis-008',
      altNeed: '示意图',
      shot: 'desktop-position-sizing.png',
      noRiskManagement: true,
    },
  ]
  ...
  for (const item of figurePages) {
    ...
    const page = await cdp.evaluate(`(() => {
      ...
      return {
        ...
        hasRiskHref: html.includes('href="/trading-system/risk-management"'),
    ...
    if (item.noRiskManagement) {
      const hasRiskLink = page.nextLinks.some(href => href && href.includes('risk-management'))
      const showsWriting = page.nextMuted.some(text => text.includes('编写中'))
      if (page.hasRiskHref || hasRiskLink) {
        fail('position-sizing 页脚链到 risk-management')
      }
      else if (!showsWriting) {
        fail(`position-sizing 页脚未显示编写中：${JSON.stringify(page.nextMuted)}`)
      }
      else {
        ok('position-sizing 页脚只显示编写中，无 risk-management href')
      }
    }
...
console.log('\nAll Sprint 006 visual checks passed')
